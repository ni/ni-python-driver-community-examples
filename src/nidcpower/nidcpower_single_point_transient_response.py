...
def example(
    ...
    plt.rcParams["figure.figsize"] = [7.50, 3.50]  # set default figure size
    # automatically adjust subplot parameters to give specified padding
    plt.rcParams["figure.autolayout"] = True
    # create a figure with 2 subplots (voltage and current)
    ...
    with nidcpower.Session(
        ...
        )  # source mode is set to single point
        session.output_function = (
            nidcpower.OutputFunction.DC_VOLTAGE
        )  # output function is set to DC voltage

        session.voltage_level = voltage_level  # set the voltage level
        session.voltage_level_range = voltage_range  # set the voltage range
        ...
        # Enable output and commit the session to apply settings
        session.output_enabled = True
        session.commit()
        # Initiate the session and measure the time taken to fetch measurements
        session.initiate()
        ...
        end_time = time.time()
        # Print the results and transient response settings
        print(
            f"Generation Time: {end_time - start_time:.6f} seconds"
        )
        ...
        x_time = [
            session.aperture_time * x for x in range(len(measurements))
        ]
        # Plot Voltage
...
def _main(argsv):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="Source delay in seconds"
    )
    parser.add_argument(
        "-tr",
        "--transient-response",
        default="CUSTOM",
        help="Transient response mode (SLOW, FAST, NORMAL, or CUSTOM)",
    )
    ...
    if args.transient_response.upper() == "CUSTOM":
        parser.add_argument(
            ...
            type=float,
            default=5000,
            help="Voltage gain bandwidth (for CUSTOM transient)",
        )
        parser.add_argument(
            "-vcf",
            "--voltage-compensation-frequency",
            type=float,
            default=50000,
            help="Voltage compensation frequency (CUSTOM)",
        )
        parser.add_argument(
            "-vpzr",
            "--voltage-pole-zero-ratio",
            type=float,
            default=0.16,
            help="Voltage pole-zero ratio (for CUSTOM transient)",
        )
        parser.add_argument(
            "-cgb",
            "--current-gain-bandwidth",
            type=float,
            default=40000,
            help="Current gain bandwidth (for CUSTOM transient)",
        )
        parser.add_argument(
            "-ccf",
            "--current-compensation-frequency",
            type=float,
            default=250000,
            help="Current compensation frequency (CUSTOM)",
        )
        parser.add_argument(
            "-cpzr",
            "--current-pole-zero-ratio",
            type=float,
            default=4,
            help="Current pole-zero ratio (for CUSTOM transient)",
        )
        ...
        if remaining_args:
            print(
                f"Warning: Custom transient parameters only used with "
                f"-tr CUSTOM mode. Ignoring: {remaining_args}"
            )
    ...
    transient_response = transient_response_map.get(
        args.transient_response.upper(), nidcpower.TransientResponse.NORMAL
    )
